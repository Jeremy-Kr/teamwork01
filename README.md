...
개발 중에는 각자 이름으로 된 브랜치를 만들고 각자 이름으로 된`branch`에 `git push`를 했습니다. 그리고 `dev`에 기능을 `merge`하고 적당한 버전에 `dev`를 `main`에 `merge`했습니다.

1.0 버전을 수요일 저녁 시점에 `dev`는 `main`에 `merge`되었습니다. 목요일 오후에 1.1 확장도 완료했습니다.
